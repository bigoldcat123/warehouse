  ...
  <el-dialog v-model="visiable" :before-close="handleClose" width="560" :show-close="false" class="dark-dialog"
    ...
    <template #header>
      <div class="dialog-header">
        ...
        <span class="text-white text-lg font-semibold">更改用户</span>
    ...
    <div class="p-6">
      <el-form ref="ruleFormRef" style="max-width: 600px" :model="ruleForm" status-icon :rules="rules" label-width="80px"
        ...
        <el-form-item label="用户名" prop="username">
          <el-input :disabled="prop.current?.username == 'admin'" v-model="ruleForm.username" placeholder="请输入用户名"
            class="dark-input" :class="{ 'dark-input--disabled': prop.current?.username == 'admin' }" />
        </el-form-item>
        <el-form-item label="密码">
          <el-input :disabled="prop.current?.username == 'admin'" type="password"
            placeholder="如果不修改密码，请保持密码框为空" v-model="ruleForm.password" class="dark-input"
            :class="{ 'dark-input--disabled': prop.current?.username == 'admin' }" />
        </el-form-item>
