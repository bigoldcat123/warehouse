<template>
    ...
    <ImageDialog :dialog-visible="imageDialog" @close="imageDialog = false" @refresh="fetchData" :urlInfo="urlInfo">
    </ImageDialog>
</template>
<script setup lang="ts">
import { ref } from 'vue'
import ImageDialog from '../house/ImageDialog.vue';
import UpdateDialog from '../house/UpdateDialog.vue';
import AddDialog from '../house/AddDialog.vue';
...
const size = ref(1000)
function fetchData() {
    house.listTongfeng(currentpage.value, size.value, warehouseName.value, houseNo.value).then(res => {
