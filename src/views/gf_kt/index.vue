...
<div class="bg-[#03153d] h-screen overflow-auto relative ">
    <div class=" h-12 bg-[#03153d] sticky top-0 flex items-center justify-center z-50">
        ...
        <img  src="/gfkt/gf_title.png" />
    </div>
    <div class=" w-[80%] mx-auto mt-28">
         <!-- <NhlInfo v-for="item in data" :key="item.id" :nhl="item" /> -->
         <GfktInfo v-for="item in data" :key="item.houseName" :info="item" />
    </div>
</div>
</template>
<script setup lang="ts">
import GfktInfo from '@/components/gfkt/GfktInfo.vue';
import gfkt,{type GfktItem} from '@/api/gf_kt';
import { onMounted, ref ,onUnmounted} from  'vue';
import type { ElButton } from 'element-plus/lib';

const data = ref<GfktItem[]>([]);

async function fetchData() {
    data.value = (await gfkt.list()).data.value;
}
fetchData();
